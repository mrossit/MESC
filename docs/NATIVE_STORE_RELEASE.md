...
- iOS native mission/formation/data candidate: build `5.4.7 (50463)` preparado para validacao; substitui o icone clinico da Missao por servico liturgico, torna Formacao resiliente a falhas de carga com recarga explicita e mostra todos os modulos disponiveis. O Supabase nativo recebeu 120 cadastros, 107 respostas e 62 escalas historicas do MESC atual, sem blobs de foto ou vinculos familiares incompletos.
...
- TestFlight: build `5.4.7 (50462)` permanece `VALID` e elegivel para testes internos. O build `50463` sucede essa versao apos upload e processamento. A API da Apple recusou associacao manual ao grupo interno `MESC Interno` (`ENTITY_UNPROCESSABLE`) em tentativa anterior, pois builds internos nao sao adicionados por esse relacionamento; validar a disponibilidade diretamente no TestFlight/App Store Connect.
