...
- TestFlight: build `5.4.7 (50448)` enviado ao App Store Connect contra `https://saojudastadeu.app`; processamento `VALID`. A API da Apple recusou associacao manual ao grupo interno `MESC Interno` (`ENTITY_UNPROCESSABLE`) em tentativa anterior, pois builds internos nao sao adicionados por esse relacionamento; validar a disponibilidade diretamente no TestFlight/App Store Connect.
