...
- Version: `5.4.3`
- Native build number / Android version code: `50418`
...
- iOS native coordinator questionnaire candidate: build `5.4.3 (50418)` validado e enviado ao App Store Connect em 23/06/2026, delivery UUID `1eedd172-ed10-4783-888c-99f74c23f40b`; inclui o painel nativo do coordenador, respostas de questionario, pendentes e diagnostico de cadastro/readiness apos merge do PR `#44`.
...
- TestFlight: build `5.4.3 (50417)` em estado `VALID`, `APP_STORE_ELIGIBLE`, associado ao grupo interno `MESC Interno`; build `5.4.3 (50418)` enviado e aguardando processamento da Apple para liberar o teste interno do fluxo nativo de questionario do coordenador.
- DNS nativo: Vercel ja possui alias para `saojudastadeu.app` e `www.saojudastadeu.app`, mas o dominio ainda resolve no provedor externo para `34.111.179.208`. Antes de distribuir o build `50418` como teste fiel, ajustar no DNS do registrar: `A saojudastadeu.app 76.76.21.21` e `A www.saojudastadeu.app 76.76.21.21`, ou trocar nameservers para `ns1.vercel-dns.com` e `ns2.vercel-dns.com`.
