...
- TestFlight: build `5.4.7 (50463)` esta `VALID` e elegivel para testes internos, sucedendo o `50462`. A API da Apple recusou associacao manual ao grupo interno `MESC Interno` (`ENTITY_UNPROCESSABLE`) em tentativa anterior, pois builds internos nao sao adicionados por esse relacionamento; validar a disponibilidade diretamente no TestFlight/App Store Connect.
