...
- TestFlight: build `5.4.7 (50462)` foi enviado contra `https://saojudastadeu.app`, processado como `VALID` e elegivel para testes internos; ele sucede o `50461`. A API da Apple recusou associacao manual ao grupo interno `MESC Interno` (`ENTITY_UNPROCESSABLE`) em tentativa anterior, pois builds internos nao sao adicionados por esse relacionamento; validar a disponibilidade diretamente no TestFlight/App Store Connect.
