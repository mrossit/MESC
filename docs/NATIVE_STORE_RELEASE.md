...
- iOS native substitution center candidate: build `5.4.7 (50461)` validado, exportado, enviado e processado pelo App Store Connect em 25/07/2026; adiciona central SwiftUI para ver pedidos abertos, acompanhar os proprios pedidos, assumir uma escala com confirmacao e abrir a tela diretamente por notificacao de substituicao. Delivery/build ID `c65f3e71-b68b-470e-a7df-44f80d505f60`, estado `VALID` e elegivel para testes internos no TestFlight.
- iOS native profile candidate: build `5.4.7 (50462)` validado localmente, exportado, enviado e processado pelo App Store Connect em 25/07/2026; adiciona edicao nativa do cadastro, foto por camera/biblioteca, permissoes de dispositivo sob demanda e mutacoes idempotentes no contrato mobile de perfil. Delivery/build ID `8088bf9d-c47a-40d9-9197-622698fe4230`, estado `VALID` e elegivel para testes internos no TestFlight.
- iOS native mission/formation/data candidate: build `5.4.7 (50463)` validado, exportado, enviado e processado pelo App Store Connect em 25/07/2026; substitui o icone clinico da Missao por servico liturgico, torna Formacao resiliente a falhas de carga com recarga explicita e mostra todos os modulos disponiveis. O Supabase nativo recebeu 120 cadastros, 107 respostas e 62 escalas historicas do MESC atual, sem blobs de foto ou vinculos familiares incompletos. Delivery/build ID `67a3ad2b-ba4e-4b2e-964e-73705d533cff`, estado `VALID` e elegivel para testes internos no TestFlight.
- iOS native mission crucifix candidate: build `5.4.7 (50464)` validado, exportado, enviado e processado pelo App Store Connect em 26/07/2026; substitui o icone de maos da aba Missao pelo crucifixo nativo do iOS, preservando o icone de maos apenas na trilha pratica de Formacao. Delivery/build ID `25ff3c01-c5c8-4164-8ba1-0851d3622650`, estado `VALID` e elegivel para testes internos no TestFlight.
- iOS native service-first candidate: build `5.4.7 (50466)` validado, exportado, enviado e processado pelo App Store Connect em 21/09/2026; apresenta a entrada do ministro como `Servir` e `Seu servico`, troca a iconografia ambigua de cruz/maleta por um simbolo de servico e pela marca MESC no cabecalho, e mantem a mesma linguagem no fallback web. Build ID `b5cc900b-6038-4e64-9279-58104735b979`, estado `VALID` e elegivel para testes internos no TestFlight.
- iOS native minister schedule candidate: build `5.4.7 (50467)` validado, exportado, enviado e processado pelo App Store Connect em 21/09/2026; apresenta a entrada `Ministrare`, transforma a marca em acesso a identidade da comunidade, remove o sino duplicado, mostra a escala mensal publicada da comunidade com detalhe de missa, P1-P28 e equipe completa, agrupa trocas por dia/horario, permite a P1/P2 editar apenas a missa que serve com auditoria/idempotencia, restaura a exportacao mensal oficial e torna o progresso da Formacao explicito. Delivery/build ID `d02f13aa-4812-428b-bcc1-927fb1d4aade`, estado `VALID` e elegivel para testes internos no TestFlight.
- iOS native launch recovery candidate: build `5.4.7 (50468)` validado, exportado, enviado e processado pelo App Store Connect em 21/09/2026; normaliza duplicidades historicas de confirmacao no payload de escalas, torna a restauracao de sessao tolerante a IDs repetidos e estabiliza os IDs de missas/posicoes usados pela lista SwiftUI. Delivery/build ID `82bf54d2-76d4-4671-b2e1-2fdb9004a0d1`, estado `VALID` e elegivel para testes internos no TestFlight.
...
- TestFlight: build `5.4.7 (50464)` esta `VALID` e elegivel para testes internos, sucedendo o `50463`. A API da Apple recusou associacao manual ao grupo interno `MESC Interno` (`ENTITY_UNPROCESSABLE`) em tentativa anterior, pois builds internos nao sao adicionados por esse relacionamento; validar a disponibilidade diretamente no TestFlight/App Store Connect.
- TestFlight: build `5.4.7 (50466)` esta `VALID` e elegivel para testes internos, sucedendo o `50464` como candidato mais recente.
- TestFlight: build `5.4.7 (50467)` esta `VALID` e elegivel para testes internos, sucedendo o `50466` como candidato mais recente.
- TestFlight: build `5.4.7 (50468)` esta `VALID` e elegivel para testes internos, sucedendo o `50467` como candidato mais recente.
...
- Supabase security: `native_data_api_rls_hardening` foi aplicado em 25/07/2026. Todas as tabelas de `public` estao com RLS ativo e sem leitura para os papeis `anon`/`authenticated`; o acesso permanece somente pela API do servidor. Os avisos `rls_enabled_no_policy` sao esperados nesse modelo fechado, pois nao existe acesso direto pelo Data API.
...
O build `5.4.7 (50452)` substitui o `50451` no TestFlight como candidato SwiftUI nativo com acoes conectadas: presenca, substituicao, exportacao HTML da escala oficial, preferencias do aparelho, avisos/pendencias reais, videos de formacao e resumo de perfil. O build `5.4.7 (50453)` adiciona a passada visual de Liquid Glass nativo e corrige o corte/faixa acima da tab bar. O build `5.4.7 (50456)` leva o estudio nativo de formacao para coordenadores/gestores e publica os contratos mobile admin correspondentes. O build `5.4.7 (50457)` troca o rodape customizado por tab bar nativa e unifica o cabecalho das telas com vidro flutuante. O build `5.4.7 (50458)` refina a experiencia interna de Escalas e Formacao para reduzir a sensacao de blocos visuais soltos.

## Push nativo APNS/FCM

O app iOS ja possui Push Notifications habilitado no Bundle ID `app.saojudastadeu.mesc` e o IPA `5.4.7 (50458)` foi assinado com `aps-environment=production`.
