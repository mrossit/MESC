...
- iOS native profile candidate: build `5.4.7 (50462)` validado localmente, exportado e enviado ao App Store Connect em 25/07/2026; adiciona edicao nativa do cadastro, foto por camera/biblioteca, permissoes de dispositivo sob demanda e mutacoes idempotentes no contrato mobile de perfil. Delivery UUID `8088bf9d-c47a-40d9-9197-622698fe4230`, aguardando processamento da Apple.
...
- TestFlight: build `5.4.7 (50461)` permanece `VALID` e elegivel para testes internos. O build `50462` foi enviado contra `https://saojudastadeu.app` e o processamento pela Apple esta pendente; ele sucede o `50461` assim que receber estado `VALID`. A API da Apple recusou associacao manual ao grupo interno `MESC Interno` (`ENTITY_UNPROCESSABLE`) em tentativa anterior, pois builds internos nao sao adicionados por esse relacionamento; validar a disponibilidade diretamente no TestFlight/App Store Connect.
