...
export function HeaderUltimasConexoes() {
  ...
  const displayData = wsConnected ? connections : (queryData || []);

  // Abrir WhatsApp
  const handleWhatsAppClick = useCallback((whatsapp: string) => {
    if (!whatsapp) return;
    ...
    <div className="header-ultimas-conexoes">
      {displayData.map((conn) => (
        <div 
          key={conn.user_id} 
          className="uc-item"
          title={`${conn.nome} - ${conn.email}`}
        >
          <div className="uc-item-icon-wrapper">
            {conn.avatar_url ? (
              <img 
                src={conn.avatar_url} 
                alt={conn.nome} 
              ...
              <div className="uc-avatar uc-avatar-placeholder">
                {conn.nome.charAt(0).toUpperCase()}
              </div>
            )}
            <span 
              className={`uc-status ${conn.status === 'online' ? 'online' : 'offline'}`}
              aria-label={conn.status === 'online' ? 'Online' : 'Offline'}
            />
          </div>
          <div className="uc-info">
            <span className="uc-name" title={conn.nome}>
              {conn.nome.length > 12 ? conn.nome.substring(0, 12) + '...' : conn.nome}
            </span>
            <span className="uc-time">{conn.last_seen_human}</span>
          </div>
          {conn.whatsapp && (
            <button
              className="uc-whatsapp"
              onClick={() => handleWhatsAppClick(conn.whatsapp!)}
