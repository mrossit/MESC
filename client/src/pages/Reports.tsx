    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Acompanhe as métricas e o desempenho do ministério
          </p>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-2xl font-bold">
                  {summaryLoading ? <Skeleton className="h-8 w-16" /> : summary?.activeMinisters || 0}
                </p>
                <p className="text-xs text-muted-foreground">Total cadastrado</p>
              </div>
              <Users className="h-8 w-8 text-neutral-accentWarm" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-2xl font-bold">
                  {summaryLoading ? <Skeleton className="h-8 w-16" /> : summary?.avgAvailabilityDays || 0}
                </p>
                <p className="text-xs text-muted-foreground">Dias por mês</p>
              </div>
              <Calendar className="h-8 w-8 text-dark-gold" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-2xl font-bold">
                  {summaryLoading ? <Skeleton className="h-8 w-16" /> : summary?.monthSubstitutions?.total || 0}
                </p>
                <p className="text-xs text-muted-foreground">
                  {summary?.monthSubstitutions?.approved || 0} aprovadas
                </p>
              </div>
              <RefreshCw className="h-8 w-8 text-dark-terracotta" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-2xl font-bold">
                  {summaryLoading ? <Skeleton className="h-8 w-16" /> : summary?.formationCompleted || 0}
                </p>
                <p className="text-xs text-muted-foreground">Módulos este mês</p>
              </div>
              <BookOpen className="h-8 w-8 text-dark-copper" />
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="availability" className="space-y-4">
        ...
        <TabsContent value="availability" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Top 10 Ministros Mais Disponíveis</CardTitle>
              <CardDescription>
        ...
        <TabsContent value="substitutions" className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card>
              <CardHeader>
                <CardTitle>Distribuição de Substituições</CardTitle>
            ...
            <Card>
              <CardHeader>
                <CardTitle>Ministros Mais Confiáveis</CardTitle>
                <CardDescription>
                  ...
                  <div className="space-y-2">
                    {substitutions?.reliableServers?.slice(0, 5).map((minister: any, index: number) => (
                      <div key={index} className="flex items-center justify-between p-2 rounded-lg bg-muted/50">
                        <div className="flex items-center gap-2">
                          <Trophy className={`h-5 w-5 ${index === 0 ? 'text-yellow-500' : index === 1 ? 'text-gray-400' : index === 2 ? 'text-orange-600' : 'text-muted-foreground'}`} />
        ...
        <TabsContent value="engagement" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Ministros Mais Engajados</CardTitle>
              <CardDescription>
                    ...
                    <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            Última atividade: {user.lastActivity ? format(new Date(user.lastActivity), "dd/MM HH:mm", { locale: ptBR }) : 'N/A'}
                          </p>
                        </div>
                      </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Taxa de Resposta aos Questionários</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between">
                <div>
        ...
        <TabsContent value="formation" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Progresso de Formação</CardTitle>
              <CardDescription>
        ...
        <TabsContent value="families" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Famílias MESC Mais Ativas</CardTitle>
              <CardDescription>
